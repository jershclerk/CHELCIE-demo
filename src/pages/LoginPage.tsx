...
export function LoginPage({ onEnterApp }: { onEnterApp: (member: OrgMember) => void }) {
  const [selectedMember, setSelectedMember] = useState<string | null>(null)
  const [isAddMemberOpen, setIsAddMemberOpen] = useState(false)
  const activeMember = members.find((member) => member.name === selectedMember) ?? null
          ...
          <div className="relative flex w-full flex-col items-start gap-4">
            ...
            >
              ...
              <span className="flex flex-col gap-0.5">
                ...
                <span className="text-xs text-muted-foreground">You will be able to do this later too</span>
              </span>
            </button>
          </div>

          {activeMember && (
            <Button type="button" className="w-full" onClick={() => onEnterApp(activeMember)}>
              Go to CHELCIE
            </Button>
          )}
        </div>
